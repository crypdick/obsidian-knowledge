...
def build_reason(vault_root: str, session_key: str | None = None) -> str:
    ...
    return (
        ...
        "File only a durable, novel delta that changes future decisions, is not already captured, "
        "and is not cheaply recoverable from code, git, issues, logs, or existing notes. "
        "Search the vault first and state the one-sentence delta missing from the canonical note. "
        "The vault is largely AI-generated and may be stale or contain hallucinated claims. "
        "Verify claims before saving them as facts; preserve uncertainty and source provenance "
        "when evidence is incomplete. Repetition across notes is not independent verification. "
        "If you cannot state it, do not file. For qualifying material, follow the "
        "remember-conversations skill; prefer updating the canonical note and create at most one "
        "durable wiki note unless the user requested more or two topics are independently reusable. "
        "Skip routine progress, transient status/PIDs/job IDs/worktrees, and generic answers. "
        "Only after changing durable vault content, create or reuse one terse same-session fragment "
        f"in {changelog_dir}/; do not log code, git, or host changes by themselves, and do not edit a "
        f"shared changelog index.{changelog_reuse} "
        "Also repair verified stale instructions already encountered in this task, following the "
        "obsidian-knowledge skill's instruction-repair rules: edit the canonical source, verify the "
        "correction, preserve user policy and safeguards, and start no new audit. "
        "If no capture or instruction repair qualifies, stop silently without writing anything."
    )
